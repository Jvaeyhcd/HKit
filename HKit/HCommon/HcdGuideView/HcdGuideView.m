...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.pageIndicatorTintColor = [UIColor colorWithRed:0.937 green:0.937 blue:0.937 alpha:1.00];
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:0.647 green:0.647 blue:0.647 alpha:1.00];
    }
    return _pageControl;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == self.images.count - 1) {
        ...
        if (self.btnImage) {
            [cell.button setImage:self.btnImage forState:UIControlStateNormal];
            [cell.button setBackgroundColor:[UIColor clearColor]];
            [cell.button.layer setBorderWidth:0.0];
        } else {
